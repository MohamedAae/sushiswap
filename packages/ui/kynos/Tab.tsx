    ...
    return (
      ...
      <HeadlessTab as={Fragment} ref={ref}>
        {({ selected }) => (
          <Button
            size="sm"
            className={classNames(className, 'hover:ring-0 hover:text-accent focus:ring-0 outline-none ') + (selected ? ' text-accent' : 'text-disabled')}
            variant={selected ? 'empty' : 'empty'}
            {...props}
          >
            {/* @ts-ignore*/}
            {children}
